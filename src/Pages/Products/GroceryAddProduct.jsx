...
import {
  ...
  FaChevronDown, FaGripLinesVertical, FaUndo, FaTrash, FaClock,
} from 'react-icons/fa';
...
const DRAFT_KEY_PREFIX = 'groceryAddDraft_';
...
const GroceryAddProduct = () => {
  ...
  const draftKey = `${DRAFT_KEY_PREFIX}${categoryType}`;
    ...
    Promise.all([
      ...
      try {
        ...
        if (raw) {
          ...
          if (parsed && (parsed.form?.name || parsed.previews?.length)) {
  ...
  // Autosave draft (debounced) so a refresh / accidental tab close never loses work
  useEffect(() => {
    if (loadingMeta || draftBanner) return;
    const hasContent = form.name.trim() || form.description.trim() || previews.length > 0;
    ...
    saveTimer.current = setTimeout(() => {
      ...
      } catch (_) { /* storage full / disabled — fail silently */ }
    }, 700);
    return () => clearTimeout(saveTimer.current);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form, specifications, productOptions, previews, categoryId, subCategoryId, subSubCategoryId, loadingMeta, draftBanner]);

  const applyDraft = () => {
    const d = pendingDraft.current;
    if (!d) return;
    setForm({ ...emptyForm, ...d.form });
    setSpecifications(d.specifications?.length ? d.specifications : [{ key: '', value: '' }]);
    setProductOptions(d.productOptions?.length ? d.productOptions : [{ name: '', label: '', values: [] }]);
    setPreviews(d.previews || []);
    setCategoryId(d.categoryId || '');
    setSubCategoryId(d.subCategoryId || '');
    setSubSubCategoryId(d.subSubCategoryId || '');
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true);
    postData('/api/go-market/products', payload).then((res) => {
      setIsLoading(false);
      if (res?.error === false || res?.success === true) {
        context.alertBox('success', res?.message || 'Grocery product added to your shop!');
        clearDraftStorage();
        context.setIsOpenFullScreenPanel({ open: false });
        history('/products');
      } else {
        context.alertBox('error', res?.message || 'Could not add product');
      }
    }).catch(() => {
      setIsLoading(false);
      context.alertBox('error', 'Could not add product. Please try again.');
    });
  };
  ...
  return (
    <section className="ga-page">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .ga-draft-banner {
          ...
          background: #fffbeb; border: 1px solid #fde68a; color: #92400e; border-radius: 12px; padding: 12px 16px;
          margin-bottom: 20px; font-size: 13px; animation: gaFadeUp 0.4s ease both;
        }
        .ga-draft-actions { display: flex; gap: 8px; }
        .ga-btn-ghost {
          display: inline-flex; align-items: center; gap: 6px; border: 1px solid currentColor; background: transparent;
          padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 700; cursor: pointer; color: inherit;
          transition: transform 0.15s ease, background 0.15s ease;
        }
        .ga-btn-ghost:hover { background: rgba(0,0,0,0.05); transform: translateY(-1px); }
        ...
        .ga-input, .ga-textarea {
          width: 84%; border: 1px solid var(--accent-border); border-radius: 10px; padding: 0 14px; font-size: 14px;
        ...
        .ga-preview-image { height: 180px; display: flex; align-items: center; justify-content: center; background-size: cover; background-position: center; transition: background-image 0.3s ease; }
        .ga-preview-body { padding: 16px; }
        .ga-preview-name { font-size: 16px; font-weight: 700; color: var(--ink); margin-bottom: 6px; transition: opacity 0.2s ease; }
        ...
        .ga-spinner-inline { animation: gaSpin 0.9s linear infinite; }
        ...
        @media (max-width: 420px) {
          ...
          .ga-input, .ga-textarea, .ga-page .MuiOutlinedInput-root { width: 78% !important; }
        }
        ...
        @media (max-width: 640px) {
          ...
          .ga-input, .ga-textarea { width: 80%; padding-left: 12px; padding-right: 12px; }
      ...
      <div className="ga-shell-inner">
        ...
        {draftBanner && (
          <div className="ga-draft-banner">
            <span><FaUndo size={13} style={{ marginRight: 8 }} />You have an unsaved draft from a previous session. Restore it?</span>
            <div className="ga-draft-actions">
              <button type="button" className="ga-btn-ghost" onClick={applyDraft}><FaUndo size={11} /> Restore</button>
              <button type="button" className="ga-btn-ghost" onClick={discardDraft}><FaTrash size={11} /> Discard</button>
            </div>
          </div>
        )}
          ...
          <div className="ga-layout">
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="ga-card ga-shop-banner">
                ...
                <div>
                  <div className="ga-shop-eyebrow">Listing in shop</div>
              ...
              <div className="ga-card">
                ...
                <div className="ga-field" ref={(el) => (fieldRefs.current.name = el)}>
                  <label className="ga-label" htmlFor="ga-name">Product name *</label>
                  <input id="ga-name" className={inputClass(!!errors.name)} name="name" value={form.name} onChange={onChange}
                ...
                <div className="ga-field">
                  <label className="ga-label" htmlFor="ga-title">Display title (product page)</label>
                  <input id="ga-title" className="ga-input" name="title" value={form.title} onChange={onChange}
                ...
                <div className="ga-field" ref={(el) => (fieldRefs.current.description = el)}>
                  <div className="ga-field-row">
                    <label className="ga-label" htmlFor="ga-description">Description *</label>
                    <span className={`ga-char-count${form.description.length > DESCRIPTION_LIMIT ? ' warn' : ''}`}>
                      {form.description.length}/{DESCRIPTION_LIMIT}
                    </span>
                  </div>
                  <textarea id="ga-description" name="description" value={form.description} onChange={onChange}
                ...
                <div className="ga-field">
                  <label className="ga-label" htmlFor="ga-productType">Product type</label>
                  <input id="ga-productType" className="ga-input" name="productType" value={form.productType} onChange={onChange}
              ...
              <div className="ga-card">
                ...
                {seoOpen && (
                  <div className="ga-seo-body">
                    <div className="ga-field">
                      <label className="ga-label" htmlFor="ga-searchKeywords">Search keywords</label>
                      <input id="ga-searchKeywords" className="ga-input" name="searchKeywords" value={form.searchKeywords} onChange={onChange}
                    ...
                    <div className="ga-field">
                      <label className="ga-label" htmlFor="ga-tags">Product tags</label>
                      <input id="ga-tags" className="ga-input" name="tags" value={form.tags} onChange={onChange}
                        placeholder="e.g. organic, fresh, local, seasonal (comma separated)" />
                    </div>
                    <div className="ga-field">
                      <label className="ga-label" htmlFor="ga-keywords">SEO keywords</label>
                      <input id="ga-keywords" className="ga-input" name="keywords" value={form.keywords} onChange={onChange}
                        placeholder="e.g. grocery, vegetables, fresh produce (comma separated)" />
                    </div>
                    <div className="ga-field">
                      <div className="ga-field-row">
                        <label className="ga-label" htmlFor="ga-seoDescription">SEO description</label>
                        <span className={`ga-char-count${form.seoDescription.length > SEO_DESC_LIMIT ? ' warn' : ''}`}>
                          {form.seoDescription.length}/{SEO_DESC_LIMIT}
                        </span>
                      </div>
                      <textarea id="ga-seoDescription" name="seoDescription" value={form.seoDescription} onChange={onChange}
                        placeholder="Short description for search engines (150-160 characters recommended)"
                        className="ga-textarea" style={{ height: 80 }} />
                    </div>
                    <div className="ga-field">
                      <label className="ga-label" htmlFor="ga-attributes">Product attributes</label>
                      <textarea id="ga-attributes" name="attributes" value={form.attributes} onChange={onChange}
              ...
              <button type="submit" disabled={isLoading} className="ga-submit-btn ga-submit-desktop">
                {isLoading ? <CircularProgress size={22} sx={{ color: '#fff' }} /> : (
                  <><FaCloudUploadAlt size={18} /> Publish to my grocery shop</>
                )}
              </button>
            </form>
        ...
        <div className="ga-sticky-bar">
          <div className="ga-sticky-bar-inner">
            ...
            <button type="button" className="ga-sticky-submit" disabled={isLoading} onClick={handleSubmit}>
              {isLoading ? <CircularProgress size={16} sx={{ color: '#fff' }} /> : <FaCloudUploadAlt size={15} />}
              Publish
            </button>
          </div>
        </div>
...
export default GroceryAddProduct;